...
int main(int argc, char *argv[]) {
    ...
    // Start measuring serial part execution time
    ...
    // End measuring serial part execution time
    double serial_end_time = omp_get_wtime();

    // Start measuring parallel part execution time
    double parallel_start_time = omp_get_wtime();

    // Generate Mandelbrot set in parallel
    ...
    // End measuring parallel part execution time
    ...
    printf("Serial part execution time: %f seconds\n", serial_end_time - serial_start_time);
    printf("Parallel part execution time: %f seconds\n", parallel_end_time - parallel_start_time);
    printf("Total execution time: %f seconds\n", total_end_time - total_start_time);

    // Write image to file
    write_pgm_image("mandelbrot_image.pgm", iteration_counts, n_x, n_y, MAX_ITERATIONS_INT);
